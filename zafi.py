...
"""%s
 ...
 - facebook    : facebook.com/Mahbub.Alam.Khan.002
 - fanspage    : facebook.com/BICP.Team.Help.Zone
 - Team Name    : Bangladesh Islamic Cyber Protector
 - github      : github.com/BlackPanther02
 - CEO : Sourav Hasan
 - version     : 1.0
...
def publik(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s]Type  Friend list public  id code or Type '%sme%s'  "%(P,M,P,H,P))
        idt = raw_input(' [*] id code  : %s'%(K))
        ...
        for a in z['friends']['data']:
            ...
            print '\r%s [*] Total id :%s %s ' % (P,H,str(len(id))),
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] ID Name %s%s'%(P,H,P,H,nm['name']))
